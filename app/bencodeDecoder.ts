import type { Dictionary, BencodeDecoderStatic } from "./types";

class BencodeDecoder {
  private constructor() {}

  static decodeBencodeString(bencodedValue: string): [string, number] {
  ...
  static decodeBencodeInteger(bencodedValue: string): [number, number] {
    // Check integer
  ...
  static decodeBencodeList(bencodedValue: string): [Array<any>, number] {
    ...
    if (bencodedValue.startsWith("l")) {
      ...
      do {
        if (valueString.startsWith("i")) {
          const [value, decodedNumberLength] =
            BencodeDecoder.decodeBencodeInteger(valueString);
        ...
        if (!isNaN(parseInt(valueString[0]))) {
          const [value, decodedStringLength] =
            this.decodeBencodeString(valueString);
          arrayBuffer.push(value);
        ...
        if (valueString.startsWith("l")) {
          const [array, decodedSubArrayLength] =
            BencodeDecoder.decodeBencodeList(valueString);
        ...
        if (valueString.startsWith("d")) {
          const [dictonary, decodeValueLegth] =
            BencodeDecoder.decodeBencodeDictonary(valueString);
  ...
  static decodeBencodeDictonary(bencodedValue: string): [Dictionary, number] {
    ...
    if (bencodedValue.startsWith("d")) {
      do {
        ...
        const [key, decodedKeyLength] =
          BencodeDecoder.decodeBencodeString(valueString);
        ...
        if (valueString.startsWith("l")) {
          const [listValue, decodedValueLength] =
            BencodeDecoder.decodeBencodeList(valueString);
          decodedLength += decodedValueLength;
          valueString = valueString.substring(decodedValueLength);
        ...
        if (!isNaN(parseInt(valueString[0]))) {
          const [value, decodedValueLength] =
            BencodeDecoder.decodeBencodeString(valueString);
  ...
  static decodeBencode(
    ...
    if (bencodedValue.startsWith("l"))
      [value, decodedSize] = BencodeDecoder.decodeBencodeList(bencodedValue);
    else if (bencodedValue.startsWith("i"))
      [value, decodedSize] = BencodeDecoder.decodeBencodeInteger(bencodedValue);
    else if (bencodedValue.startsWith("d"))
      [value, decodedSize] =
        BencodeDecoder.decodeBencodeDictonary(bencodedValue);
    else
      [value, decodedSize] = BencodeDecoder.decodeBencodeString(bencodedValue);

    return value;
  }
}

const _staticCheck: BencodeDecoderStatic = BencodeDecoder;
